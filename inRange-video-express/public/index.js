...
function successCallback(stream) {
  ...
  select.addEventListener('input', (evt) => handleChangeInput(evt.target.value));

  function handleChangeInput(color) {
  ...
  socket.on('audio', (checked) => {
    document.getElementById('audio-check').checked = canPlayAudio = checked;
  });
  ...
  socket.on('select', (color) => {
    select.value = color;
    handleChangeInput(select.value);
  });

  socket.on('input', ({ min, max }) => {
    renderInput(min, max);
  });
  ...
  video.oncanplay = () => {
    ...
    function processVideo() {
      ...
      socket.emit('video', {
        dataURL: {
          src: srcCanvas.toDataURL('image/png'),
          dist: distCanvas.toDataURL('image/png')
        },
        ratio
      });
    }
